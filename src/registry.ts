...
export const commands: CommandEntry[] = [
  {
    ...
    input: {
      ...
      flags: [
        ...
        { name: "--all", type: "boolean", summary: "Remove all exclusions: include hidden, gitignored, and normally-excluded files.", default: false },
      ],
    },
  ...
  {
    path: ["files"],
    summary: "List file paths with git-aware scope and bundled ripgrep.",
    effect: "read",
    input: {
      positionals: [{ name: "query", required: false, variadic: true }],
      flags: [
        { name: "--query", type: "string", summary: "Query for files-with-matches mode. Use this when the query itself starts with a dash." },
        { name: "--root", type: "string", summary: "Root path to list. Defaults to the current working directory." },
        { name: "--glob", type: "string[]", summary: "Restrict paths with ripgrep glob patterns." },
        { name: "--type", type: "string[]", summary: "Restrict paths with ripgrep file type filters." },
        { name: "--max", type: "number", summary: "Maximum files to return.", default: 1000 },
        { name: "--timeoutMs", type: "number", summary: "Wall-clock timeout in milliseconds.", default: 5000 },
        { name: "--regex", type: "boolean", summary: "Treat the query as a regular expression.", default: false },
        { name: "--all", type: "boolean", summary: "Remove all exclusions: include hidden, gitignored, and normally-excluded files.", default: false },
      ],
    },
    output: { documented: true, schema: "FilesEnvelope" },
    examples: [
